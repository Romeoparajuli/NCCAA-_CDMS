...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO users (name, email, password, role, member_type, district_id, province_id) VALUES (?, ?, ?, ?, ?, ?, ?)");

        // Retrieve district and province IDs based on names
        $district_id = null;
        if ($district_name) {
            $district_stmt = $conn->prepare("SELECT id FROM districts WHERE name = ?");
            $district_stmt->bind_param("s", $district_name);
            $district_stmt->execute();
            $district_stmt->bind_result($district_id);
            $district_stmt->fetch();
            $district_stmt->close();
        }
        ...
        $province_id = null;
        if ($province_name) {
            $province_stmt = $conn->prepare("SELECT id FROM provinces WHERE name = ?");
            $province_stmt->bind_param("s", $province_name);
            $province_stmt->execute();
            $province_stmt->bind_result($province_id);
            $province_stmt->fetch();
            $province_stmt->close();
        }

        $stmt->bind_param("sssssii", $name, $email, $hashed_password, $role, $member_type, $district_id, $province_id);
