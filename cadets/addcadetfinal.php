...
$stmt = $pdo->prepare("INSERT INTO cadets (name, date_of_birth, gender, blood_group, contact_number, email, 
facebook_link, father_name, mother_name, father_contact, mother_contact, permanent_address, temporary_address, 
education_qualification, see_school, ncc_cadet_number, rank, division, ncc_batch, ncc_year, ncc_school, 
active_status, district_id, province_id) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

// Execute the statement with the correct number of parameters
if ($stmt->execute([$name, $date_of_birth, $gender, $blood_group, $contact_number, $email, $facebook_link, 
...
$active_status, $district_id, $province_id])) {
    // Handle the trainings separately, if needed
    
    echo "<div class='bg-green-500 text-white p-4 rounded mb-4'>Cadet added successfully!</div>";
    echo "<script>
        setTimeout(function() {
    ...
    echo "<div class='bg-red-500 text-white p-4 rounded mb-4'>Error adding cadet: " . htmlspecialchars($stmt->errorInfo()[2]) . "</div>";
}
}
?>
        ...
        <form method="POST" action="">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" class="input-field rounded p-2 w-full" placeholder="Enter email address" required>
                </div>
                <div>
                    <label for="facebook_link" class="block font-medium">Facebook Link:</label>
                    <input type="url" name="facebook_link" id="facebook_link" class="input-field rounded p-2 w-full" placeholder="Enter Facebook profile link">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <textarea name="permanent_address" id="permanent_address" class="input-field rounded p-2 w-full" placeholder="Enter permanent address" required></textarea>
                </div>
                <div>
                    <label for="temporary_address" class="block font-medium">Temporary Address:</label>
                    <textarea name="temporary_address" id="temporary_address" class="input-field rounded p-2 w-full" placeholder="Enter temporary address" required></textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    <label for="see_school" class="block font-medium">School Attended:</label>
                ...
                <div>
                    ...
                    <select name="division" id="division" class="input-field rounded p-2 w-full" required>
            ...
            <div class="flex justify-center">
                <button type="submit" class="btn-theme rounded">Add Cadet</button>
            </div>
        </form>
    </div>
